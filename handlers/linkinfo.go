...
func (m *Repository) LinkInfo(w http.ResponseWriter, r *http.Request) {
	...
	if rq.BrowserCode == "" {
		...
		if uEBool {
			...
			infoQ1, err := m.DataBase.Query("SELECT COUNT(*), `link`, `name`, `type`, `des`, `thuimg`, `preimg`, `active`, `time` FROM `linka_links` WHERE `username` = '" + eUserName + "' AND `pageid` = " + fmt.Sprint(pageId) + " AND `slug` = '" + rq.LinkSlug + "' AND `active` IN (1, 2, 3)")
			if err != nil {
				log.Println(err)
				jsonRes.Success = false
				json.NewEncoder(w).Encode(&jsonRes)
			...
			for infoQ1.Next() {
				infoQ1.Scan(&jsonRes.LinkInfo.LinkCount, &jsonRes.LinkInfo.LinkURI, &jsonRes.LinkInfo.LinkName, &jsonRes.LinkInfo.LinkType, &jsonRes.LinkInfo.LinkDes, &jsonRes.LinkInfo.ThuImg, &jsonRes.LinkInfo.PreImg, &jsonRes.LinkInfo.LinkVisible, &jsonRes.LinkInfo.LinkTime)
			}
